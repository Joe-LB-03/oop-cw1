 ...
 *
 * @author Nick Efford & Louis Bishop
 */
public class Point 
{
    ...
    {
        if(lon <= MAX_LONGITUDE && lon >= MIN_LONGITUDE && lat <= MAX_LATITUDE && lat >= MIN_LATITUDE)
        {
            time = t;
            latitude = lat;
            longitude = lon;
        ...
        {
            throw new GPSException("Invalid latitude and longitude");
        }
    }
